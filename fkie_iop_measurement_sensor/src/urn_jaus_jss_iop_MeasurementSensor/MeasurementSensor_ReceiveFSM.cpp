...
void MeasurementSensor_ReceiveFSM::setupNotifications()
{
	...
	p_measurement_sub = cfg.subscribe<fkie_iop_msgs::Measurement>("measurement", 1, &MeasurementSensor_ReceiveFSM::measurementReceived, this);
...
void MeasurementSensor_ReceiveFSM::measurementReceived(const fkie_iop_msgs::Measurement::ConstPtr& measurement)
{
	...
	for (unsigned int i = 0; i < measurement->values.size() && i < 255; i++) {
		...
		fkie_iop_msgs::MeasurementValue mval = measurement->values[i];
