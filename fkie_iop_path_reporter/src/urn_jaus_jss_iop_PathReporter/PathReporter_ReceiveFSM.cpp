...
{
    double dist = std::sqrt(std::pow(first.pose.position.x - second.pose.position.x, 2) + std::pow(first.pose.position.y - second.pose.position.y, 2) + std::pow(first.pose.position.z - second.pose.position.z, 2));
    if (dist > p_min_dist) {
        return true;
    }
    try {
...
bool PathReporter_ReceiveFSM::p_transform_pose(geometry_msgs::msg::PoseStamped& pose, std::string target_frame)
{
    try {
        if (pose.header.frame_id.compare(target_frame) != 0) {
            p_tf_buffer->lookupTransform(p_tf_frame_robot, pose.header.frame_id, tf2::TimePointZero, tf2::durationFromSec(0.5));
